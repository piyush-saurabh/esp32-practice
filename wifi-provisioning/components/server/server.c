...
static esp_err_t on_setwifi_set(httpd_req_t *req)
{
    ...
    //"ssid=xxxx\r\npass=xxxx\r\n"
    // Split up the string using strtok
    // this will replace 1st occurance to null. T
    // The pointer will point to 's' of ssid. 
    char *ssid = strtok(buf, "\r\n"); // Result: "ssid=xxxx\0"

    // "ssid=xxxx\0pass=xxxx\r\n"
    // strtok "remembers" the string. On passing NULL, it will operate on the same string (buf)
    // The pointer will point to 'p' of pass. 
    char *pass = strtok(NULL, "\r\n"); // Result: "pass=xxxx\0"

    // Extract ssid value from "ssid=xxxx\0"
    ssid = strchr(ssid, '=') + 1;

    // Extract pass value from "pass=xxxx\0"
    pass = strchr(pass, '=') + 1;
...
void RegisterEndPoints(void)
{
    ...
    // Endpoint for setting up wifi creds
    httpd_uri_t setwifi_end_point_config = {
        .uri = "/api/setwifi",
        .method = HTTP_POST,
        .handler = on_setwifi_set // handler callback function once this URL is hit
    };
    // Register the handler with the server
