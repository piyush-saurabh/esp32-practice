...
void append_file(char *path, char *content);
void read_file(char *path);

void log_to_sdcard(char *filename, char *content)
{
    ...
    // char file_path[30];
    // sprintf(file_path, "%s/%s", BASE_PATH, filename);
    // append_file(file_path, content);

    append_file(filename, content);
    
    // append_file("/store/logs-1.csv", "13:00,3.5\n");
    // append_file("/store/logs-1.csv", "14:15,3.6\n");
    // append_file("/store/logs-1.csv", "15:66,4.2\n");
...
void append_file(char *path, char *content)
{
    ESP_LOGI(TAG, "Appending \"%s\" to \"%s\"", content, path);

    FILE *file = fopen(path, "a+");

    if (!file) file = fopen(path, "w+");
    if (!file) {
        ESP_LOGI(TAG, "Can not open the file \"%s\" for writing.", path);
        return;
    }
    ...
    ESP_LOGI(TAG, "Append successfull!|");
}
